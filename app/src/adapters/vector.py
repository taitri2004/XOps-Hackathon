...
class BedrockKBVector:
    ...
    def delete_doc(self, doc_id: str, filter: Optional[dict] = None) -> None:
        # Bedrock KB removal happens after the S3 object is deleted and the KB data
        # source is re-synced. The app cannot synchronously delete vector chunks.
        return None
...
class LocalVector:
    ...
    def delete_doc(self, doc_id: str, filter: Optional[dict] = None) -> None:
        def keep(item: tuple[str, str, dict]) -> bool:
            _, _, md = item
        ...
        self.docs = [item for item in self.docs if keep(item)]
