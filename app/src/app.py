...
async def strip_api_prefix(request: Request, call_next):
    """Let the same app work behind CloudFront /api/* and locally at /."""
...
@app.post("/summarize")
def summarize(req: DocRequest, x_user_id: str | None = Header(default=None)) -> dict:
    user_id = _resolve_user_id(x_user_id)
    try:
        return handlers.handle_summarize(user_id, req.doc_id, ai_client, userstore, storage)
    except KeyError as e:
        raise HTTPException(status_code=404, detail=str(e))


@app.post("/quiz")
def quiz(req: QuizRequest, x_user_id: str | None = Header(default=None)) -> dict:
    user_id = _resolve_user_id(x_user_id)
    try:
        return handlers.handle_quiz(user_id, req.doc_id, ai_client, userstore, storage, n=req.n)
    except KeyError as e:
        raise HTTPException(status_code=404, detail=str(e))
