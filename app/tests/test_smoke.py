...
def test_api_prefix_routes_work_for_cloudfront():
...
def test_summarize_and_quiz_uploaded_doc():
    upload = client.post(
        "/upload",
        files={
            "file": (
                "biology.txt",
                b"Photosynthesis converts light energy into chemical energy. Chlorophyll absorbs light in plant leaves. Glucose stores energy for later use.",
                "text/plain",
            )
        },
        headers={"X-User-Id": "study-features"},
    )
    ...
    assert len(summary.json()["concepts"]) == 5

    quiz = client.post(
        "/quiz",
        json={"doc_id": doc_id, "n": 3},
        headers={"X-User-Id": "study-features"},
    )
...
def test_delete_doc_removes_doc_from_listing():
    upload = client.post(
        "/upload",
        files={"file": ("delete-me.txt", b"This document should disappear after delete.", "text/plain")},
        headers={"X-User-Id": "delete-user"},
    )
    doc_id = upload.json()["doc_id"]

    delete = client.delete(f"/docs/{doc_id}", headers={"X-User-Id": "delete-user"})
